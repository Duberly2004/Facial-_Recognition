from db.conection import Prisma
...
class TeacherController:
    ...
    async def create(self,request):
        ...
        functions = Functions()
        email = functions.remove_acents(f'{name.split(" ")[0][0].lower()}{lastname.split(" ")[0].lower()}@tecsup.edu.pe')
        ...
        user_type = await self.prisma.user_type.find_unique(where={"name":EUserType.TEACHER})
        ...
        await self.prisma.teacher.create(data={
            "user":{
                "create": {
                        "name":name,
                        "lastname":lastname,
    ...
    async def list(self):
        await self.prisma.connect()
        response = await self.prisma.teacher.find_many(include={"user":True})
        data = [
            {
                "id":item.id,
    ...
    def delete():
        print("delete teachers")
        return jsonify({"message":"Delete teachers"})